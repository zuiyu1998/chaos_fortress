...
`BattleState` 是一个组件（Component），用于存储战斗实体的**战斗属性**数据，包括当前血量和最大血量。

## 用途

- 持有 `BattleState` 组件的实体代表一个可参与战斗的单位（角色或敌人），拥有血量等属性。
...
impl BattleState {
    ...
    /// 从 BattleAttributeSet 生成 BattleState，提取当前 hp、max_hp。
...
| 方法 | 说明 |
|------|------|
| `new(max_hp)` | 创建一个新的 `BattleState`，`hp` 初始化为 `max_hp`（满血）。 |
| `from_attribute_set(set)` | 从 `BattleAttributeSet` 生成 `BattleState`，提取当前 `hp`、`max_hp`。 |
| `is_dead() -> bool` | 如果 `hp ≤ 0` 返回 `true`，表示实体已死亡。 |
| `take_damage(raw_damage)` | 承受原始伤害 `raw_damage`，直接作用于 `hp`（最少为 0）。 |
...
- **敌人模块**（`enemy`）：敌人实体同样可通过 `BattleState` 组件拥有血量。
